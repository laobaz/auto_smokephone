...
public:
    ...
    void solve(Matrix<Data> &m, int camID) {
        ...
        long long monitorCnt=0;
        // Follow the steps
        int step = 1;
        while ( step  && monitorCnt < 100000) {
        ...
        if(monitorCnt>=100000)
        {
          ANNIWOLOG(INFO) << "Munkres::Solve monitorCnt too big."<<"camID:"<<camID;
        }
