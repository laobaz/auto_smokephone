...
void tracker::update(const DETECTIONS & detections,int camID)
{
    TRACHER_MATCHD res;
    _match(detections, res,camID);
...
void tracker::update(const DETECTIONSV2 & detectionsv2,int camID)
{
    ...
    ANNIWOLOG(INFO) << "tracker::update _match success:"<<res.matches.size()<<","<<res.unmatched_tracks.size()<<","<<res.unmatched_detections.size()<<"camID:"<<camID;
    #endif
  ...
  for (Track & track:tracks) {
    ...
    #ifdef ANNIWO_INTERNAL_DEBUG
    ANNIWOLOG(INFO) << "tracker::update partial_fit before"<<"camID:"<<camID;
    #endif

    this->metric->partial_fit(tid_features, active_targets);

    #ifdef ANNIWO_INTERNAL_DEBUG
    ANNIWOLOG(INFO) << "tracker::update partial_fit after"<<"camID:"<<camID;
    #endif

}

void tracker::_match(const DETECTIONS & detections, TRACHER_MATCHD & res,int camID)
{
  ...
  for (Track & t:tracks) {
    ...
    #ifdef ANNIWO_INTERNAL_DEBUG
    ANNIWOLOG(INFO) << "tracker::_match before matching_cascade"<<","<<"camID:"<<camID;
    #endif
    ...
    #ifdef ANNIWO_INTERNAL_DEBUG
    ANNIWOLOG(INFO) << "tracker::_match after matching_cascade"<<","<<"camID:"<<camID;
    #endif
...
void tracker::_initiate_track(const DETECTION_ROW& detection, CLSCONF clsConf)
{
    ...
    this->tracks.push_back(Track(mean, covariance, this->_next_idx, this->n_init, 
                            this->max_age, detection.feature, clsConf.cls, clsConf.conf, detection.uuid));
    _next_idx += 1;
}
