...
static const int new_NUM_CLASSES = 5; //
...
static const int new_YOLO4_OUTPUT_SIZE = 1 * 7581 * NUM_ANCHORS * (new_NUM_CLASSES + 5);
...
static std::vector<std::string> new_PHONEclass_names = {
    "smoke",
    "smoke_hand",
    "phone",
    "phone_hand",
...
void SmokePhoneDetection::detect(int camID, int instanceID, cv::Mat img, const Polygon *polygonSafeArea_ptr, const std::vector<Object> &in_person_results, std::string &f)
{
        ...
        if (isInsideP)
        {
            ...
            if (iterCamInstance != executionContexts.end())
            {
                yolov4_detection_staff(m_input_datas, camID, instanceID, faceimg2,
                                       ...
                                       new_YOLO4_OUTPUT_SIZE, INPUT_W, INPUT_H, objects, BBOX_CONF_THRESH, new_NUM_CLASSES,
            ...
            bool found = true;
             for(auto obj:objects){
                     ANNIWOLOGF(INFO, "SmokePhoneDetection all on face:camID:%d  %s = confidence:%.5f at x:%.2f y:%.2f w:%.2f  h:%.2f\n", camID, new_PHONEclass_names[obj.label].c_str(), obj.prob,
                                   obj.rect.x, obj.rect.y, obj.rect.width, obj.rect.height);
            ...
            for (auto &kv : globalINICONFObj.in_use_conf_functions)
            {
                ...
                if (kv== "smoke")
                {
                    ...
                    {
                        Object obj = objects[i];
                        bool find_smoke = false;
                        ANNIWOLOGF(INFO, "SmokePhoneDetection smoke on face:camID:%d  %s = confidence:%.5f at x:%.2f y:%.2f w:%.2f  h:%.2f\n", camID, PHONEclass_names[obj.label].c_str(), obj.prob,
                                   obj.rect.x, obj.rect.y, obj.rect.width, obj.rect.height);
                        if (globalINICONFObj.ANNIWO_CHANGE_SMOKE2PHONE == 1)
                        {

                            if (obj.label == 1 || obj.label == 2 || obj.label == 3 || obj.label == 4) // 类别为：smoke smoke_hand phone phone_hand background
                            {
                                // 确保有smoke
                                continue;
                            }
                            ...
                            for (auto obj1 : objects)
                            {
                                ...
                                if (obj1.label == 1)
                                {
                                    float inter_area = intersection_area(obj1, obj);
                        ...
                        {
                            if (obj.label == 2 || obj.label == 3 || obj.label == 4)
                            { // 类别为：smoke smoke_hand phone phone_hand background

                                continue;
                            }
                            find_smoke = true;
                        }
                        // 当smoke是在smoke_hand中说明
                        if (find_smoke)
                        {
                            // 交换
                ...
                else if (kv== "phone")
                {
                    ...
                    for (size_t i = 0; i < objects.size(); i++)
                    {
                        ...
                        if (globalINICONFObj.ANNIWO_CHANGE_SMOKE2PHONE == 1)
                        {
                            if (obj.label == 0 || obj.label == 1 || obj.label == 3 || obj.label == 4) // 类别为：smoke smoke_hand phone phone_hand background
                            {                                                                         // 确保有phone
                                continue;
                            }
                        ...
                        {
                            if (obj.label == 0 || obj.label == 1 || obj.label == 4)
                            // 类别为：smoke smoke_hand phone phone_hand background
                            {
                                continue;
                            }
    ...
    if (overall_objects.size() > 0)
    {
        ...
        save_imgXmL(img,overall_objects,f, new_PHONEclass_names);
